...
OperationTreeNode *newOperationTreeNode(const char *label, uint32_t childCount,
                                        uint32_t line, uint32_t pos,
                                        bool isImaginary) {
  OperationTreeNode *node =
      (OperationTreeNode *)malloc(sizeof(OperationTreeNode));
  node->label = strdup(label);
  node->childCount = childCount;
  node->children =
      (OperationTreeNode **)malloc(childCount * sizeof(OperationTreeNode *));
  node->line = line;
  node->pos = pos;
...
bool isBinaryOp(const char *label) {
  return strcmp(label, PLUS) == 0 || strcmp(label, MINUS) == 0 ||
         strcmp(label, MUL) == 0 || strcmp(label, DIV) == 0 ||
         strcmp(label, MOD) == 0 || strcmp(label, LE) == 0 ||
         strcmp(label, GR) == 0 || strcmp(label, LE_EQ) == 0 ||
         strcmp(label, GR_EQ) == 0 || strcmp(label, EQ) == 0 ||
         strcmp(label, NEQ) == 0 || strcmp(label, AND) == 0 ||
         strcmp(label, OR) == 0;
}
...
bool isArithmeticOp(const char *label) {
  return strcmp(label, PLUS) == 0 || strcmp(label, MINUS) == 0 ||
         strcmp(label, MUL) == 0 || strcmp(label, DIV) == 0 ||
         strcmp(label, MOD) == 0 || strcmp(label, NEG) == 0;
}

bool isLogicalOp(const char *label) {
  return strcmp(label, AND) == 0 || strcmp(label, OR) == 0 ||
         strcmp(label, NOT) == 0;
}

bool isEqOp(const char *label) {
  return strcmp(label, EQ) == 0 || strcmp(label, NEQ) == 0;
}

bool isCmpOp(const char *label) {
  return strcmp(label, LE) == 0 || strcmp(label, GR) == 0 ||
         strcmp(label, LE_EQ) == 0 || strcmp(label, GR_EQ) == 0 ||
         strcmp(label, EQ) == 0 || strcmp(label, NEQ) == 0;
}

bool isUnaryOp(const char *label) {
  return strcmp(label, NEG) == 0 || strcmp(label, NOT) == 0;
}

bool isLiteral(const char *label) {
  return strcmp(label, BOOL) == 0 || strcmp(label, STR) == 0 ||
         strcmp(label, SYMB) == 0 || strcmp(label, HEX) == 0 ||
         strcmp(label, BITS) == 0 || strcmp(label, DEC) == 0;
}
...
bool isSymbType(const char *type) { return strcmp(type, "char") == 0; }

bool isTextType(const char *type) { return strcmp(type, "string") == 0; }
...
bool isBinaryOperationAllowed(const char *op, const char *lType,
                              const char *rType) {
  if ((strcmp(op, PLUS) == 0) && (strcmp(lType, "string") == 0) &&
      (strcmp(rType, "string") == 0)) {
    return true;
  }
  if (isArithmeticOp(op)) {
    ...
    return (isNumericType(lType) && isNumericType(rType)) ||
           (isLogicalType(lType) && isLogicalType(rType));
...
bool isUnaryOperationAllowed(const char *op, const char *type) {
  return (isNumericType(type) || isLogicalType(type)) &&
         (isArithmeticOp(op) || isLogicalOp(op));
}
...
void checkTypeCompatibility(OperationTreeNode *lValueExprNode,
  ...
  if (!lValueExprNode->type->custom && !lValueExprNode->type->isArray) {
    if (strcmp(lValueExprNode->type->typeName, "byte") == 0) {
      ...
      if (strcmp(rValueExprNode->label, LIT_READ) == 0 &&
        ...
        rValueExprNode->type = newTypeParent;
        rValueExprNode->children[1]->type = newTypeChild;
      } else if (strcmp(rValueExprNode->type->typeName, "long") != 0) {
        char buffer[1024];
        snprintf(buffer, sizeof(buffer),
      ...
      if (strcmp(rValueExprNode->label, LIT_READ) == 0 &&
        ...
        rValueExprNode->type = newTypeParent;
        rValueExprNode->children[1]->type = newTypeChild;
      } else if (strcmp(rValueExprNode->type->typeName, "ulong") != 0) {
        char buffer[1024];
        snprintf(buffer, sizeof(buffer),
      ...
      if (strcmp(rValueExprNode->type->typeName, "int") != 0 &&
          strcmp(rValueExprNode->type->typeName, "char") != 0) {
        char buffer[1024];
        snprintf(buffer, sizeof(buffer),
  ...
  if (lValueExprNode->type->isArray) {
    ...
    } else if (rValueExprNode->type->isArray &&
               (lValueExprNode->type->arrayDim !=
                rValueExprNode->type->arrayDim)) {
      char buffer[1024];
      snprintf(buffer, sizeof(buffer),
               "Assign error. Can't assign %s to array %s:%d:%d\n",
               rValueExprNode->type->typeName, filename, rValueExprNode->line,
  ...
  if (rValueExprNode->type->isArray) {
    if (!lValueExprNode->type->isArray) {
      ...
      snprintf(buffer, sizeof(buffer),
               ...
               lValueExprNode->type->typeName, filename, rValueExprNode->line,
               rValueExprNode->pos + 1);
      if (container->error == NULL) {
        container->error = createOperationTreeErrorInfo(buffer);
      } else {
...
char *convertHexOrBitToDecimalString(const char *input, NumberType type,
  ...
  if (strcmp(root->label, ASSIGN) == 0) {
    ...
    if (root->childCount == 2) {
      ...
      if (func != NULL) {
        if (!func->isVarargs) {
          if (callNode->childCount - 1 == func->argumentsCount) {
            for (uint32_t i = callNode->childCount - 1; i > 0; i--) {
              if (func != NULL) {
                ...
                OperationTreeNode *fakeArgNode = newOperationTreeNode(
            ...
            char buffer[1024];
            snprintf(buffer, sizeof(buffer),
                     "Call error. Different count of parameters and arguments "
                     "in calling function %s at %s:%d:%d\n",
          ...
          if (callNode->childCount - 1 >= func->argumentsCount) {
            ArgumentInfo **argArray =
                listToArray(func->arguments, func->argumentsCount);
            for (uint32_t i = 0; i < func->argumentsCount; i++) {
              OperationTreeNode *fakeArgNode =
                  newOperationTreeNode(argArray[i]->name, 0, argArray[i]->line,
            ...
            for (uint32_t i = func->argumentsCount; i < callNode->childCount;
                 i++) {
              OperationTreeNode *fakeArgNode = newOperationTreeNode(
      ...
      if (isLvalue) {
        ...
        snprintf(
            ...
            filename, callNode->line, callNode->pos + 1);
        if (container->error == NULL) {
          container->error = createOperationTreeErrorInfo(buffer);
        } else {
          addOperationTreeError(container, buffer);
    ...
    if (root->childCount == 1) {
      ...
      snprintf(buffer, sizeof(buffer),
               ...
               indexNameNode->line, indexNameNode->pos + 1);
      if (container->error == NULL) {
        container->error = createOperationTreeErrorInfo(buffer);
      } else {
        addOperationTreeError(container, buffer);
      ...
      if (indexNode->type->isArray) {
        ...
        indexNode->type = createTypeInfo("char", false, false, 0,
                                         indexNode->line, indexNode->pos);
        // indexNameNode->type = createTypeInfo("char", false, true, 1,
        // indexNode->line, indexNode->pos);
      }
      if (!(indexNameNode->type->isArray ||
            strcmp(indexNameNode->type->typeName, "string") == 0)) {
        char buffer[1024];
        snprintf(buffer, sizeof(buffer),
                 "Index error. Index can be applied only for arrays or strings "
                 "at %s:%d:%d\n",
                 filename, indexNameNode->line, indexNameNode->pos + 1);
        if (container->error == NULL) {
          container->error = createOperationTreeErrorInfo(buffer);
        } else {
          addOperationTreeError(container, buffer);
      ...
      for (uint32_t i = 0; i < root->children[0]->childCount; i++) {
        ...
        if (strcmp(iNode->label, WRITE) == 0) {
          ...
          snprintf(buffer, sizeof(buffer),
                   ...
                   "%s:%d:%d\n",
                   filename, iNode->line, iNode->pos + 1);
          if (container->error == NULL) {
            container->error = createOperationTreeErrorInfo(buffer);
          } else {
            addOperationTreeError(container, buffer);
        ...
        if (strcmp(iNode->type->typeName, "int") != 0) {
          char buffer[1024];
          snprintf(buffer, sizeof(buffer),
                   "Index error. Only int type can be used for indexing at "
                   "%s:%d:%d\n",
                   filename, iNode->line, iNode->pos + 1);
          if (container->error == NULL) {
            container->error = createOperationTreeErrorInfo(buffer);
    ...
    if (leftExprNode->type->isArray || rightExprNode->type->isArray ||
        leftExprNode->type->custom || rightExprNode->type->isArray) {
      binaryOpNode->type = createTypeInfo(
          "_", false, false, 0, binaryOpNode->line, binaryOpNode->pos);
      char buffer[1024];
      snprintf(buffer, sizeof(buffer),
               "Operation error. Can't apply binary operation '%s' for custom "
               "type or array %s at %s:%d:%d\n",
      ...
      if (strcmp(leftExprNode->type->typeName, rightExprNode->type->typeName) ==
          0) {
        if (isBinaryOperationAllowed(binaryOpNode->label,
                                     leftExprNode->type->typeName,
                                     rightExprNode->type->typeName)) {
          if (isCmpOp(binaryOpNode->label)) {
            binaryOpNode->type = createTypeInfo(
                "bool", false, false, 0, binaryOpNode->line, binaryOpNode->pos);
          } else {
          ...
          char buffer[1024];
          snprintf(buffer, sizeof(buffer),
                   "Operation error. Can't apply binary operation '%s' for "
                   "type %s at %s:%d:%d\n",
                   binaryOpNode->label, leftExprNode->type->typeName, filename,
                   binaryOpNode->line, binaryOpNode->pos + 1);
          if (container->error == NULL) {
        ...
        char buffer[1024];
        snprintf(buffer, sizeof(buffer),
                 "Type error. Can't perform binary operation '%s' with "
                 "different types (%s and %s) at %s:%d:%d\n",
                 binaryOpNode->label, leftExprNode->type->typeName,
                 rightExprNode->type->typeName, filename, binaryOpNode->line,
                 binaryOpNode->pos + 1);
        if (container->error == NULL) {
          container->error = createOperationTreeErrorInfo(buffer);
        } else {
    ...
    if (isUnaryOperationAllowed(unaryOpNode->label, exprNode->type->typeName)) {
      ...
      char buffer[1024];
      snprintf(buffer, sizeof(buffer),
               "Operation error. Can't apply unary operation '%s' for type %s "
               "at %s:%d:%d\n",
               unaryOpNode->label, exprNode->type->typeName, filename,
               exprNode->line, exprNode->pos + 1);
      if (container->error == NULL) {
        container->error = createOperationTreeErrorInfo(buffer);
      } else {
        addOperationTreeError(container, buffer);
    ...
    OperationTreeNode *idValueNode = newOperationTreeNode(
        root->children[0]->label, 0, root->children[0]->line,
        root->children[0]->pos, root->children[0]->isImaginary);
    ...
    if (!isFunctionName) {
      if (gotSymbol == NULL) {
        ...
        idValueNode->type = createTypeInfo(
    ...
    if (isLvalue) {
      ...
      snprintf(buffer, sizeof(buffer),
               ...
               filename, root->children[0]->line, root->children[0]->pos + 1);
      addOperationTreeError(container, buffer);
    }
    if (isFunctionName) {
      char buffer[1024];
    ...
    OperationTreeNode *literalValueNode = newOperationTreeNode(
        root->children[0]->label, 0, root->children[0]->line,
        root->children[0]->pos, root->children[0]->isImaginary);
    ...
    if (strcmp(literalTypeNode->label, SYMB) == 0) {
      litReadNode->children[1]->type =
          createTypeInfo("char", false, false, 0, root->children[0]->line,
                         root->children[0]->pos);
    } else if (strcmp(literalTypeNode->label, BOOL) == 0) {
      litReadNode->children[1]->type =
          createTypeInfo("bool", false, false, 0, root->children[0]->line,
                         root->children[0]->pos);
      ...
      if (!isCanBeInt(literalValueNode->label)) {
        if (isCanBeUnsignedInt(literalValueNode->label)) {
          litReadNode->children[1]->type =
              createTypeInfo("uint", false, false, 0, root->children[0]->line,
                             root->children[0]->pos);
        } else if (isCanBeLong(literalValueNode->label)) {
          litReadNode->children[1]->type =
              createTypeInfo("long", false, false, 0, root->children[0]->line,
                             root->children[0]->pos);
        } else if (isCanBeUnsignedLong(literalValueNode->label)) {
          litReadNode->children[1]->type =
              createTypeInfo("ulong", false, false, 0, root->children[0]->line,
                             root->children[0]->pos);
        } else {
          litReadNode->children[1]->type =
              createTypeInfo("_", false, false, 0, root->children[0]->line,
          ...
          char buffer[1024];
          snprintf(buffer, sizeof(buffer),
                   "Type error. Can't represent a value %s as int, uint, long "
                   "or ulong at %s:%d:%d\n",
        ...
        litReadNode->children[1]->type =
            createTypeInfo("int", false, false, 0, root->children[0]->line,
                           root->children[0]->pos);
      }
    } else if (strcmp(literalTypeNode->label, HEX) == 0) {
      char *output = (char *)malloc(100);
      char *converted = convertHexOrBitToDecimalString(
          literalValueNode->label, CONV_TYPE_INT, output, sizeof(output));
      if (converted == NULL) {
        char buffer[1024];
        snprintf(buffer, sizeof(buffer),
                 "Represantation error. Can't represent a hex value %s as int, "
                 "uint, long or ulong at %s:%d:%d\n",
                 literalValueNode->label, filename, root->children[0]->line,
                 root->children[0]->pos + 1);
        if (container->error == NULL) {
          container->error = createOperationTreeErrorInfo(buffer);
        ...
        if (!isCanBeInt(converted)) {
          if (isCanBeUnsignedInt(converted)) {
            ...
            litReadNode->children[1]->type =
                createTypeInfo("ulong", false, false, 0,
                               root->children[0]->line, root->children[0]->pos);
          } else {
            litReadNode->children[1]->type =
                createTypeInfo("_", false, false, 0, root->children[0]->line,
                               root->children[0]->pos);
            char buffer[1024];
            snprintf(buffer, sizeof(buffer),
                     "Type error. Can't represent a hex value %s as int, uint, "
                     "long or ulong at %s:%d:%d\n",
                     literalValueNode->label, filename, root->children[0]->line,
                     root->children[0]->pos + 1);
            if (container->error == NULL) {
              container->error = createOperationTreeErrorInfo(buffer);
      ...
      if (converted == NULL) {
        char buffer[1024];
        snprintf(buffer, sizeof(buffer),
                 "Represantation error. Can't represent a binary value %s as "
                 "int, uint, long or ulong at %s:%d:%d\n",
                 literalValueNode->label, filename, root->children[0]->line,
                 root->children[0]->pos + 1);
        if (container->error == NULL) {
          container->error = createOperationTreeErrorInfo(buffer);
        ...
        if (!isCanBeInt(converted)) {
          if (isCanBeUnsignedInt(converted)) {
            ...
            char buffer[1024];
            snprintf(buffer, sizeof(buffer),
                     "Type error. Can't represent a binary value %s as int, "
                     "uint, long or ulong at %s:%d:%d\n",
                     literalValueNode->label, filename, root->children[0]->line,
                     root->children[0]->pos + 1);
            if (container->error == NULL) {
      ...
      litReadNode->children[1]->type =
          createTypeInfo("string", false, false, 0, root->children[0]->line,
                         root->children[0]->pos);
    }
    litReadNode->type =
        createTypeInfo(litReadNode->children[1]->type->typeName, false, false,
                       0, root->children[0]->line, root->children[0]->pos);
    return litReadNode;
  } else {
    return NULL;
  }
...
OperationTreeNode *buildTyperefHelper(OperationTreeErrorContainer *container,
  ...
  if (varType->isArray) {
    ...
    if (varType->next != NULL) {
      withTypeNode->children[2] =
          newOperationTreeNode(OT_ARRAY, 2, varType->line, varType->pos, false);
      char buffer[12];
      snprintf(buffer, sizeof(buffer), "%u", varType->arrayDim);
      withTypeNode->children[2]->children[0] =
          newOperationTreeNode(buffer, 0, varType->line, varType->pos, false);

      withTypeNode->children[2]->children[1] =
          buildTyperefHelper(container, varType->next, filename);

    } else {
      withTypeNode->children[2] =
          newOperationTreeNode(OT_ARRAY, 1, varType->line, varType->pos, false);
      char buffer[12];
      snprintf(buffer, sizeof(buffer), "%u", varType->arrayDim);
      withTypeNode->children[2]->children[0] =
          newOperationTreeNode(buffer, 0, varType->line, varType->pos, false);
...
OperationTreeNode *buildVarOperationTreeFromAstNode(
  ...
  if (varCount == 1) {
    ...
    if (gotSymbol == NULL) {
      addSymbol(sm, symbolName, varType->typeName, varType->custom,
                varType->isArray, varType->arrayDim,
                root->children[1]->children[0]->line,
                root->children[1]->children[0]->pos);
      varNode->children[1]->type = copyTypeInfo(varType);
    } else {
      char buffer[1024];
      snprintf(buffer, sizeof(buffer),
               "Var declaration error. Var with name %s at %s:%d:%d was "
               "previously declared at %s:%d:%d\n",
               symbolName, filename, root->children[1]->children[0]->line,
               root->children[1]->children[0]->pos + 1, filename,
               gotSymbol->line, gotSymbol->pos + 1);
      if (container->error == NULL) {
        container->error = createOperationTreeErrorInfo(buffer);
      } else {
        addOperationTreeError(container, buffer);
    ...
    while (i + 1 < root->childCount) {
      ...
      if (gotSymbol == NULL) {
        addSymbol(sm, symbolName, varType->typeName, varType->custom,
                  varType->isArray, varType->arrayDim,
                  root->children[i]->children[0]->line,
                  root->children[i]->children[0]->pos);
        ...
        snprintf(buffer, sizeof(buffer),
                 ...
                 root->children[i]->children[0]->pos + 1, filename,
                 gotSymbol->line, gotSymbol->pos + 1);
        if (container->error == NULL) {
          container->error = createOperationTreeErrorInfo(buffer);
        } else {
...
void printOperationTreeHelper(OperationTreeNode *node, int level) {
  ...
  printf(
      "%s (Line: %u, Pos: %u, Imaginary: %s, Type: %s, Array: %d, Reg: %s)\n",
      node->label, node->line, node->pos, node->isImaginary ? "Yes" : "No",
      node->type != NULL ? node->type->typeName : "_",
      node->type != NULL ? node->type->arrayDim : -1, node->reg);
...
OperationTreeErrorInfo *createOperationTreeErrorInfo(const char *message) {
  OperationTreeErrorInfo *errorInfo =
      (OperationTreeErrorInfo *)malloc(sizeof(OperationTreeErrorInfo));
  errorInfo->message = strdup(message);
  errorInfo->next = NULL;
  return errorInfo;
}

void addOperationTreeError(OperationTreeErrorContainer *container,
                           const char *buffer) {
  if (container->error == NULL) {
    container->error = createOperationTreeErrorInfo(buffer);
  } else {
...
TypeInfo *copyTypeInfo(TypeInfo *typeInfo) {
  ...
  TypeInfo *copy =
      createTypeInfo(typeInfo->typeName, typeInfo->custom, typeInfo->isArray,
                     typeInfo->arrayDim, typeInfo->line, typeInfo->pos);
...
FunctionEntry *createFunctionEntry(const char *fileName,
...
void freeFunctionEntry(FunctionEntry *entry, void (*freeValueLocals)(void *),
                       void (*freeValueConsts)(void *)) {
  if (!entry) {
    return;
  }
...
void freeFunctionTable(FunctionTable *table, void (*freeValueLocals)(void *),
                       void (*freeValueConsts)(void *)) {
  if (!table) {
    return;
  }
...
FunctionEntry *findFunctionEntry(FunctionTable *table,
                                 const char *functionName) {
